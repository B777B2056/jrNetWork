...
1、封装了socket底层系统调用；   
2、统一封装了poll/epoll，二者可以使用相同的API；   
...
│   │       │   ├── favicon.ico  
│   │       │   ├── hello.html  静态html网页  
│   │       │   └── test.jpg  1MB测试图片（测试超过OS底层TCP缓冲区的文件）  
...
## 2 API说明
### 2.1 事件服务端模型——EventDispatch  
>TODO
### 2.2 事件封装——
>TODO
### 2.3 socket封装——TCP::ClientSocket,TCP::ServerSocket,UDP::Socket
>TODO
### 2.4 线程池——jrThreadPool::ThreadPool
>TODO
### 2.5 定时器——Timer,TimerContainer
>TODO
## 3 总体架构
### 3.1 jrNetWork网络库
#### 3.1.1 概览  
#### 服务端事件处理模式——基于IO复用的单Reactor多线程模式
![基于IO复用的Reactor](pic/struct.png)
#### 网络库整体流程（已统一封装poll/epoll）
![整体流程](pic/server.png)
