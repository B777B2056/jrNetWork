...
namespace jrHTTP {
    ...
    HTTPServer::HTTPServer(uint port, std::string path, std::string work_directory, uint max_task_num, uint max_pool_size)
        ...
        for(auto& p : ret_head_table)
            ret_head += (p.first+":"+p.second+"\r\n");
    }

    void HTTPServer::add_ret_line(std::string key, std::string value) {
        if(ret_head_table.find(key) == ret_head_table.end()) {
    ...
    void HTTPServer::http_handler(jrNetWork::TCP::Socket *client) {
        ...
        std::string ret_body;
        if(ret_code == 400) {
            ret_body = status_table[ret_code];
        } else {
    ...
    std::string HTTPServer::exec_cgi(const std::string &path, const std::string &parameters, int &ret_code, std::string method) {
        ...
        if(-1 == pid) {
             ...
             if(-1 == execl(cgi_path.c_str(), cgi_path.c_str(), nullptr))
    ...
    bool HTTPServer::parser_request_line(jrNetWork::TCP::Socket *client, hash_map &request_line_table) {
    ...
    bool HTTPServer::parser_request_head(jrNetWork::TCP::Socket *client, hash_map &request_head_table) {
